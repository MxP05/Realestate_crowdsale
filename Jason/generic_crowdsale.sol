...
contract RealEstateTokenCrowdsale is Crowdsale, MintedCrowdsale {
...
contract RealEstateTokenCrowdsaleDeployer {
    ...
    public{RealEstateToken token = new RealEstateToken(name, symbol, 0);
    //store token's address in public variable
    realestate_token_address = address(token);
    //create token crowdsale and include token
    RealEstateTokenCrowdsale realestate_token_crowdsale = new RealEstateTokenCrowdsale(1, wallet, token);
